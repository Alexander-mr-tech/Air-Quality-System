...
const SensorData = () => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    ...
    return (
      <div style={{
        ...
        <p style={{ color: "rgba(255,255,255,0.5)", margin: "0 0 6px" }}>{label}</p>
        {payload.map((p) => (
          <p key={p.dataKey} style={{ color: p.color, margin: "3px 0" }}>
            {p.name}: <strong>{p.value}</strong>
          </p>
  ...
  return (
    <div style={{
      ...
      background: "var(--bg)", fontFamily: "'Inter','Segoe UI',sans-serif",
    }}>
      <Navbar />
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        <Sidebar />
        <main style={{ flex: 1, marginLeft: "240px", padding: "24px", overflowY: "auto" }}>
          <div style={{ maxWidth: "1100px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: "16px" }}>
              <div style={{ ...card, padding: "20px", background: healthAdvice.bg, border: `0.5px solid ${healthAdvice.border}` }}>
                <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                  <div style={{
                    width: "46px", height: "46px", borderRadius: "12px", flexShrink: 0,
                    background: `${healthAdvice.color}20`, border: `0.5px solid ${healthAdvice.border}`,
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "14px" }}>
              {statColors.map((s) => (
                <div key={s.key} style={{ ...card, padding: "18px", background: s.bg, border: `0.5px solid ${s.border}` }}>
            ...
            {/* Chart */}
            <div style={{ ...card, padding: "24px" }}>
              <p style={{ margin: "0 0 8px", fontSize: "14px", fontWeight: "600", color: "#fff" }}>
                Multi-Sensor Trend Analysis
              </p>
              {/* Legend labels */}
              <div style={{ display: "flex", gap: "16px", marginBottom: "16px", flexWrap: "wrap" }}>
                {[
                  ...
                  { label: "Humidity (%)", color: "var(--dot)", axis: "Right axis" },
                  ...
                  <div key={l.label} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    ...
                    <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.25)" }}>({l.axis})</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{ height: "300px" }}>
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={graphData}>
                    ...
                    {/* Left Y-axis: CO2 & MQ135 (high values) */}
                    <YAxis
                      yAxisId="left"
                      tick={{ fill: "rgba(255,255,255,0.35)", fontSize: 11 }}
                      axisLine={false}
                    ...
                    {/* Right Y-axis: Temp & Humidity (low values 0-100) */}
                    <YAxis
                      yAxisId="right"
                      orientation="right"
                      domain={[0, 100]}
                    ...
                    <Area yAxisId="left"  type="monotone" dataKey="co2"      stroke="#a78bfa" fill="url(#grad-co2)"      strokeWidth={2} name="CO2 (ppm)"    dot={false} />
                    <Area yAxisId="left"  type="monotone" dataKey="mq135"    stroke="#4ade80" fill="url(#grad-mq135)"    strokeWidth={2} name="MQ135"        dot={false} />
                    <Area yAxisId="right" type="monotone" dataKey="humidity" stroke="#4a9eff" fill="url(#grad-humidity)" strokeWidth={2} name="Humidity (%)" dot={false} />
                    <Area yAxisId="right" type="monotone" dataKey="temp"     stroke="#f87171" fill="url(#grad-temp)"     strokeWidth={2} name="Temp (°C)"    dot={false} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
